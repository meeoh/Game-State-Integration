    ...
    if (payload.map) {
      if (payload.map.round >= 0) {
        ...
        if (payload.round && payload.round.win_team) {
          if (
            payload.round.win_team.toUpperCase() ==
            payload.player.team.toUpperCase()
          ) {
            data.win = true;
          }
        }
...
app.post("/request/:name", function (req, res) {
  ...
  if (name.toLowerCase() == "shameel") {
    bot.sendMessage(
      chatId,
      "@meeoh, requesting to come upstairs, dont start another"
    );
  } else if (name.toLowerCase() == "nomar") {
    bot.sendMessage(
      chatId,
      "@nomar, requesting to come upstairs, dont start another"
    );
  } else if (name.toLowerCase() == "omas") {
    bot.sendMessage(
      chatId,
      "@Paytheo, requesting to come upstairs, dont start another"
    );
  } else if (name.toLowerCase() == "all") {
    bot.sendMessage(
      chatId,
      "/all, requesting to come upstairs, dont start another"
    );
  }
});
